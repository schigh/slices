...
func TestUIntSlice_TruncateZero(t *testing.T) {
	tests := []struct {
		name string
		slice []uint
		expected []uint
		...
		{
			name: "unaffected 2",
			slice: []uint{0,0,0,0,1},
			expected: []uint{0,0,0,0,1},
		},
		{
			name: "all but one",
			slice: []uint{1,0,0,0,0},
			expected: []uint{1},
		},
		{
			name: "last one only",
			slice: []uint{1,2,3,4,0},
			expected: []uint{1,2,3,4},
		},
		{
			name: "empties the slice",
			slice: []uint{0,0,0,0,0},
			expected: []uint{},
		},
		{
			name: "empty slice",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result := UIntSlice(test.slice).TruncateZero().Value()
