...
// TryEach
func TestInt32Slice_TryEach(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			e, i := Int32Slice(test.slice).TryEach(test.eachFunc)
			if test.expected != e {
...
// IfEach
func TestInt32Slice_IfEach(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			e, i := Int32Slice(test.slice).IfEach(test.eachFunc)
			if test.expected != e {
				t.Errorf("expected %v, got %v", test.expected, e)
			}
			if test.expected2 != i {
...
func TestInt32Slice_Chunk(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			size:     2,
			slice:    []int32{1, 2, 5, 11, 13, 15},
			expected: [][]int32{{1, 2}, {5, 11}, {13, 15}},
		},
		{
			name:     "chunks of 2 with remainder",
			size:     2,
			slice:    []int32{1, 2, 5, 11, 13, 15, 17},
			expected: [][]int32{{1, 2}, {5, 11}, {13, 15}, {17}},
		},
		{
			name:     "chunks of 100",
			size:     100,
