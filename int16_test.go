package slices

import (
	"errors"
	"fmt"
	"io/ioutil"
	"reflect"
	"testing"

	"github.com/schigh/slices/internal"
)

/* generated by github.com/schigh/slices/base/base.go.  do not edit. */

//region TESTS
// IndexOf
func TestInt16Slice_IndexOf(t *testing.T) {
	tests := []struct {
		name     string
		slice    []int16
		needle   int16
		expected int
	}{
		{
			name:     "only item",
			slice:    []int16{1},
			needle:   1,
			expected: 0,
		},
		{
			name:     "at index 1",
			slice:    []int16{0, 1, 1},
			needle:   1,
			expected: 1,
		},
		{
			name:     "missing",
			slice:    []int16{1, 2, 3, 4},
			needle:   5,
			expected: NotInSlice,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result := Int16Slice(test.slice).IndexOf(test.needle)
			if result != test.expected {
				t.Errorf("expected %d, got %d", test.expected, result)
			}
		})
	}
}

// Contains
func TestInt16Slice_Contains(t *testing.T) {
	tests := []struct {
		name     string
		slice    []int16
		needle   int16
		expected bool
	}{
		{
			name:     "present",
			slice:    []int16{1, 2, 3, 4},
			needle:   4,
			expected: true,
		},
		{
			name:     "not present",
			slice:    []int16{1, 2, 3, 4},
			needle:   5,
			expected: false,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result := Int16Slice(test.slice).Contains(test.needle)
			if result != test.expected {
				t.Errorf("expected %v, got %v", test.expected, result)
			}
		})
	}
}

// SortAsc
func TestInt16Slice_SortAsc(t *testing.T) {
	tests := []struct {
		name     string
		slice    []int16
		expected []int16
	}{
		{
			name:     "empty",
			slice:    []int16{},
			expected: []int16{},
		},
		{
			name:     "already sorted",
			slice:    []int16{0, 1, 2, 3, 4, 5},
			expected: []int16{0, 1, 2, 3, 4, 5},
		},
		{
			name:     "reversed",
			slice:    []int16{5, 4, 3, 2, 1, 0},
			expected: []int16{0, 1, 2, 3, 4, 5},
		},
		{
			name:     "random",
			slice:    []int16{3, 1, 4, 5, 0, 2},
			expected: []int16{0, 1, 2, 3, 4, 5},
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result := Int16Slice(test.slice).SortAsc().Value()
			if !reflect.DeepEqual(test.expected, result) {
				t.Errorf("expected %v, got %v", test.expected, result)
			}
		})
	}
}

// SortDesc
func TestInt16Slice_SortDesc(t *testing.T) {
	tests := []struct {
		name     string
		slice    []int16
		expected []int16
	}{
		{
			name:     "empty",
			slice:    []int16{},
			expected: []int16{},
		},
		{
			name:     "already sorted",
			slice:    []int16{5, 4, 3, 2, 1, 0},
			expected: []int16{5, 4, 3, 2, 1, 0},
		},
		{
			name:     "reversed",
			slice:    []int16{0, 1, 2, 3, 4, 5},
			expected: []int16{5, 4, 3, 2, 1, 0},
		},
		{
			name:     "random",
			slice:    []int16{3, 1, 4, 5, 0, 2},
			expected: []int16{5, 4, 3, 2, 1, 0},
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result := Int16Slice(test.slice).SortDesc().Value()
			if !reflect.DeepEqual(test.expected, result) {
				t.Errorf("expected %v, got %v", test.expected, result)
			}
		})
	}
}

// Unique
func TestInt16Slice_Unique(t *testing.T) {
	tests := []struct {
		name     string
		slice    []int16
		expected []int16
	}{
		{
			name:     "unaffected",
			slice:    []int16{0, 1, 2, 3, 4, 5},
			expected: []int16{0, 1, 2, 3, 4, 5},
		},
		{
			name:     "one extra five",
			slice:    []int16{5, 0, 1, 2, 3, 4, 5},
			expected: []int16{5, 0, 1, 2, 3, 4},
		},
		{
			name:     "extras everywhere",
			slice:    []int16{0, 0, 1, 0, 1, 2, 2, 2, 3, 0, 3, 4, 2, 3, 4, 4, 2, 1, 0},
			expected: []int16{0, 1, 2, 3, 4},
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result := Int16Slice(test.slice).Unique().Value()
			if !reflect.DeepEqual(test.expected, result) {
				t.Errorf("expected %v, got %v", test.expected, result)
			}
		})
	}
}

// Reverse
func TestInt16Slice_Reverse(t *testing.T) {
	tests := []struct {
		name     string
		slice    []int16
		expected []int16
	}{
		{
			name:     "even length",
			slice:    []int16{0, 1, 2, 3, 4, 5},
			expected: []int16{5, 4, 3, 2, 1, 0},
		},
		{
			name:     "odd length",
			slice:    []int16{0, 1, 2, 3, 4, 5, 6},
			expected: []int16{6, 5, 4, 3, 2, 1, 0},
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result := Int16Slice(test.slice).Reverse().Value()
			if !reflect.DeepEqual(test.expected, result) {
				t.Errorf("expected %v, got %v", test.expected, result)
			}
		})
	}
}

// Filter
func TestInt16Slice_Filter(t *testing.T) {
	tests := []struct {
		name       string
		slice      []int16
		expected   []int16
		filterFunc func(int16) bool
	}{
		{
			name:       "gt 10",
			slice:      []int16{1, 2, 5, 11, 13, 15},
			expected:   []int16{11, 13, 15},
			filterFunc: func(n int16) bool { return n > 10 },
		},
		{
			name:       "mod 3",
			slice:      []int16{1, 2, 6, 11, 12, 15, 17},
			expected:   []int16{6, 12, 15},
			filterFunc: func(n int16) bool { return n%3 == 0 },
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result := Int16Slice(test.slice).Filter(test.filterFunc).Value()
			if !reflect.DeepEqual(test.expected, result) {
				t.Errorf("expected %v, got %v", test.expected, result)
			}
		})
	}
}

// Each
func TestInt16Slice_Each(t *testing.T) {

	var rabbit int16
	tests := []struct {
		name     string
		slice    []int16
		expected int16
		eachFunc func(int16)
	}{
		{
			name:     "add n",
			slice:    []int16{1, 2, 5, 11, 13, 15},
			expected: 47,
			eachFunc: func(n int16) { rabbit += n },
		},
		{
			name:     "subtract n",
			slice:    []int16{1, 2, 6, 8, 12},
			expected: 18,
			eachFunc: func(n int16) { rabbit -= n },
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			Int16Slice(test.slice).Each(test.eachFunc)
			if test.expected != rabbit {
				t.Errorf("expected %v, got %v", test.expected, rabbit)
			}
		})
	}
}

// TryEach
func TestInt16Slice_TryEach(t *testing.T) {

	var rabbit int16
	myErr := errors.New("i am an error")
	tests := []struct {
		name      string
		slice     []int16
		expected  int
		expected2 error
		before    func()
		eachFunc  func(int16) error
	}{
		{
			name:      "add n",
			slice:     []int16{1, 2, 5, 11, 13, 15},
			expected:  NotInSlice,
			expected2: nil,
			eachFunc: func(n int16) error {
				rabbit += n
				return nil
			},
		},
		{
			name:      "subtract n",
			slice:     []int16{1, 2, 6, 8, 12},
			expected:  NotInSlice,
			expected2: nil,
			eachFunc: func(n int16) error {
				rabbit -= n
				return nil
			},
		},
		{
			name:      "errors",
			slice:     []int16{1, 2, 5, 11, 13, 15},
			expected:  3,
			expected2: myErr,
			before:    func() { rabbit = 0 },
			eachFunc: func(n int16) error {
				if n > 5 {
					return myErr
				}
				rabbit += n
				return nil
			},
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if test.before != nil {
				test.before()
			}
			e, i := Int16Slice(test.slice).TryEach(test.eachFunc)
			if test.expected != e {
				t.Errorf("expected %v, got %v", test.expected, e)
			}
			if test.expected2 != i {
				t.Errorf("expected %v, got %v", test.expected2, i)
			}
		})
	}
}

// IfEach
func TestInt16Slice_IfEach(t *testing.T) {

	var rabbit int16
	tests := []struct {
		name      string
		slice     []int16
		expected  int
		expected2 bool
		before    func()
		err       error
		eachFunc  func(int16) bool
	}{
		{
			name:      "all return true",
			slice:     []int16{1, 2, 5, 11, 13, 15},
			expected:  NotInSlice,
			expected2: true,
			eachFunc: func(n int16) bool {
				rabbit += n
				return true
			},
		},
		{
			name:      "subtract n",
			slice:     []int16{1, 2, 6, 8, 12},
			expected:  NotInSlice,
			expected2: true,
			eachFunc: func(n int16) bool {
				rabbit -= n
				return true
			},
		},
		{
			name:      "breaking",
			slice:     []int16{1, 2, 5, 11, 13, 15},
			expected:  3,
			expected2: false,
			before:    func() { rabbit = 0 },
			eachFunc: func(n int16) bool {
				if n > 5 {
					return false
				}
				rabbit += n
				return true
			},
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if test.before != nil {
				test.before()
			}
			e, i := Int16Slice(test.slice).IfEach(test.eachFunc)
			if test.expected != e {
				t.Errorf("expected %v, got %v", test.expected, e)
			}
			if test.expected2 != i {
				t.Errorf("expected %v, got %v", test.expected2, i)
			}
		})
	}
}

// Map
func TestInt16Slice_Map(t *testing.T) {
	tests := []struct {
		name     string
		slice    []int16
		expected []int16
		mapFunc  func(int16) int16
	}{
		{
			name:     "add 3",
			slice:    []int16{1, 2, 5, 11, 13, 15},
			expected: []int16{4, 5, 8, 14, 16, 18},
			mapFunc:  func(n int16) int16 { return n + 3 },
		},
		{
			name:     "set mod 2",
			slice:    []int16{1, 2, 6, 8, 12, 15, 17},
			expected: []int16{1, 0, 0, 0, 0, 1, 1},
			mapFunc:  func(n int16) int16 { return int16(n % 2) },
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			Int16Slice(test.slice).Map(test.mapFunc)
			if !reflect.DeepEqual(test.expected, test.slice) {
				t.Errorf("expected %v, got %v", test.expected, test.slice)
			}
		})
	}
}

// Chunk
func TestInt16Slice_Chunk(t *testing.T) {
	tests := []struct {
		name     string
		size     int
		slice    []int16
		expected [][]int16
	}{
		{
			name:     "chunks of 2 no remainder",
			size:     2,
			slice:    []int16{1, 2, 5, 11, 13, 15},
			expected: [][]int16{{1, 2}, {5, 11}, {13, 15}},
		},
		{
			name:     "chunks of 2 with remainder",
			size:     2,
			slice:    []int16{1, 2, 5, 11, 13, 15, 17},
			expected: [][]int16{{1, 2}, {5, 11}, {13, 15}, {17}},
		},
		{
			name:     "chunks of 100",
			size:     100,
			slice:    []int16{1, 2, 5, 11, 13, 15, 17},
			expected: [][]int16{{1, 2, 5, 11, 13, 15, 17}},
		},
		{
			name:     "chunks of 4",
			size:     4,
			slice:    []int16{1, 2, 5, 11, 13, 15, 17},
			expected: [][]int16{{1, 2, 5, 11}, {13, 15, 17}},
		},
		{
			name:     "chunks of 5",
			size:     5,
			slice:    []int16{1, 2, 5, 11, 13, 15, 17},
			expected: [][]int16{{1, 2, 5, 11, 13}, {15, 17}},
		},
		{
			name:     "empty slice",
			size:     5,
			slice:    []int16{},
			expected: [][]int16{},
		},
		{
			name:     "invalid chunk size",
			size:     -1,
			slice:    []int16{1, 2, 5, 11, 13, 15, 17},
			expected: [][]int16{},
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			out := Int16Slice(test.slice).Chunk(test.size)
			if !reflect.DeepEqual(test.expected, out) {
				t.Errorf("expected %v, got %v", test.expected, test.slice)
			}
		})
	}
}

// endregion

// region BENCHMARKS
// IndexOf
func BenchmarkInt16Slice_IndexOf(b *testing.B) {
	benchmarks := []struct {
		name  string
		slice []int16
	}{
		{
			name:  "10 elements",
			slice: internal.GenInt16Slice(10),
		},
		{
			name:  "100 elements",
			slice: internal.GenInt16Slice(100),
		},
		{
			name:  "1000 elements",
			slice: internal.GenInt16Slice(1000),
		},
		{
			name:  "10000 elements",
			slice: internal.GenInt16Slice(10000),
		},
		{
			name:  "100000 elements",
			slice: internal.GenInt16Slice(100000),
		},
	}
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				Int16Slice(bm.slice).IndexOf(1)
			}
		})
	}
}

// Contains
func BenchmarkInt16Slice_Contains(b *testing.B) {
	benchmarks := []struct {
		name  string
		slice []int16
	}{
		{
			name:  "10 elements",
			slice: internal.GenInt16Slice(10),
		},
		{
			name:  "100 elements",
			slice: internal.GenInt16Slice(100),
		},
		{
			name:  "1000 elements",
			slice: internal.GenInt16Slice(1000),
		},
		{
			name:  "10000 elements",
			slice: internal.GenInt16Slice(10000),
		},
		{
			name:  "100000 elements",
			slice: internal.GenInt16Slice(100000),
		},
	}
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				Int16Slice(bm.slice).Contains(1)
			}
		})
	}
}

// SortAsc
func BenchmarkInt16Slice_SortAsc(b *testing.B) {
	benchmarks := []struct {
		name  string
		slice []int16
	}{
		{
			name:  "10 elements",
			slice: internal.GenInt16Slice(10),
		},
		{
			name:  "100 elements",
			slice: internal.GenInt16Slice(100),
		},
		{
			name:  "1000 elements",
			slice: internal.GenInt16Slice(1000),
		},
		{
			name:  "10000 elements",
			slice: internal.GenInt16Slice(10000),
		},
		{
			name:  "100000 elements",
			slice: internal.GenInt16Slice(100000),
		},
	}
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				Int16Slice(bm.slice).SortAsc()
			}
		})
	}
}

// SortDesc
func BenchmarkInt16Slice_SortDesc(b *testing.B) {
	benchmarks := []struct {
		name  string
		slice []int16
	}{
		{
			name:  "10 elements",
			slice: internal.GenInt16Slice(10),
		},
		{
			name:  "100 elements",
			slice: internal.GenInt16Slice(100),
		},
		{
			name:  "1000 elements",
			slice: internal.GenInt16Slice(1000),
		},
		{
			name:  "10000 elements",
			slice: internal.GenInt16Slice(10000),
		},
		{
			name:  "100000 elements",
			slice: internal.GenInt16Slice(100000),
		},
	}
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				Int16Slice(bm.slice).SortDesc()
			}
		})
	}
}

// Reverse
func BenchmarkInt16Slice_Reverse(b *testing.B) {
	benchmarks := []struct {
		name  string
		slice []int16
	}{
		{
			name:  "10 elements",
			slice: internal.GenInt16Slice(10),
		},
		{
			name:  "100 elements",
			slice: internal.GenInt16Slice(100),
		},
		{
			name:  "1000 elements",
			slice: internal.GenInt16Slice(1000),
		},
		{
			name:  "10000 elements",
			slice: internal.GenInt16Slice(10000),
		},
		{
			name:  "100000 elements",
			slice: internal.GenInt16Slice(100000),
		},
	}
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				Int16Slice(bm.slice).Reverse()
			}
		})
	}
}

// Filter
func BenchmarkInt16Slice_Filter(b *testing.B) {
	benchFunc := func(n int16) bool {
		return n%2 == 0
	}
	benchmarks := []struct {
		name  string
		slice []int16
	}{
		{
			name:  "10 elements",
			slice: internal.GenInt16Slice(10),
		},
		{
			name:  "100 elements",
			slice: internal.GenInt16Slice(100),
		},
		{
			name:  "1000 elements",
			slice: internal.GenInt16Slice(1000),
		},
		{
			name:  "10000 elements",
			slice: internal.GenInt16Slice(10000),
		},
		{
			name:  "100000 elements",
			slice: internal.GenInt16Slice(100000),
		},
	}
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				Int16Slice(bm.slice).Filter(benchFunc)
			}
		})
	}
}

// Each
func BenchmarkInt16Slice_Each(b *testing.B) {
	var rabbit int16
	benchFunc := func(n int16) {
		rabbit = n
	}
	benchmarks := []struct {
		name  string
		slice []int16
	}{
		{
			name:  "10 elements",
			slice: internal.GenInt16Slice(10),
		},
		{
			name:  "100 elements",
			slice: internal.GenInt16Slice(100),
		},
		{
			name:  "1000 elements",
			slice: internal.GenInt16Slice(1000),
		},
		{
			name:  "10000 elements",
			slice: internal.GenInt16Slice(10000),
		},
		{
			name:  "100000 elements",
			slice: internal.GenInt16Slice(100000),
		},
	}
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				Int16Slice(bm.slice).Each(benchFunc)
			}
		})
	}

	fmt.Fprintf(ioutil.Discard, "%v", rabbit)
}

// Map
func BenchmarkInt16Slice_Map(b *testing.B) {
	benchFunc := func(n int16) int16 {
		n++
		return n
	}
	benchmarks := []struct {
		name  string
		slice []int16
	}{
		{
			name:  "10 elements",
			slice: internal.GenInt16Slice(10),
		},
		{
			name:  "100 elements",
			slice: internal.GenInt16Slice(100),
		},
		{
			name:  "1000 elements",
			slice: internal.GenInt16Slice(1000),
		},
		{
			name:  "10000 elements",
			slice: internal.GenInt16Slice(10000),
		},
		{
			name:  "100000 elements",
			slice: internal.GenInt16Slice(100000),
		},
	}
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				Int16Slice(bm.slice).Map(benchFunc)
			}
		})
	}
}

// endregion
